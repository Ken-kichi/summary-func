...
def summarize():
    """
    POSTリクエストで受け取ったニューステキストを要約する

    リクエスト: { "news_text": "記事本文" }
    レスポンス: { "summary": "要約結果" }
    """
    try:
        data = request.json
...
def convert_mermaid_png():
    ...
    try:
        ...
        return jsonify({'error': 'mmdc (mermaid-cli) がインストールされていません'}), 500
    except Exception as e:
        return jsonify({'error': f'エラーが発生しました: {str(e)}'}), 500
...
このガイドについてご質問があれば、以下の方法でお問い合わせください：

- **GitHub Issues**: バグ報告や機能リクエスト
- **ソースコード**: このガイドに付属するソースコードでご確認ください
