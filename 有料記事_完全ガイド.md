...
生成AIを使ってAI/テクノロジー系ニュースを解説する有料記事を書いています。読者から「自分でも解説アプリを作れないか？」と聞かれるたびに、コードで説明できていないことが気になり、このガイドを書き始めました。**自分でニュースを集め、要点をAIに抽出させ、Mermaidで図解し、最後はAzureにデプロイする**—そんな一連の流れを初心者エンジニアでも踏破できるように丁寧に分解しています。
...
1. **Azure OpenAI（GPT-4）** による高精度な要約生成
...
1. [前提条件の確認](#前提条件の確認)
2. [Step 1: 開発環境のセットアップ](#step-1-開発環境のセットアップ)
3. [Step 2: Azure環境の準備](#step-2-azure環境の準備)
4. [Step 3: ローカルで動作確認](#step-3-ローカルで動作確認)
5. [Step 4: GitHubにコードをアップロード](#step-4-githubにコードをアップロード)
6. [Step 5: Azure App Serviceへのデプロイ](#step-5-azure-app-serviceへのデプロイ)
7. [Step 6: CI/CDパイプラインの構築](#step-6-cicdパイプラインの構築)
8. [Step 7: トラブルシューティング](#step-7-トラブルシューティング)
...
```bash
# 全てを一度に確認する場合
python3 --version && git --version && node --version && npm --version
```
...
**インストール確認：**
```bash
pip list | grep -E "Flask|langchain|openai"
```
...
3. 以下を入力：
   ...
   - **リージョン**: `East US`（重要！GPT-5.1は限定リージョンのみ対応）
   - **価格レベル**: `Standard S0`
...
OpenAIリソース作成後、GPT-4モデルをデプロイします。

**手順：**

1. 作成した「news-summarizer-openai」リソースを開く
2. 左のメニューから**Model deployments** → **Manage Deployments**をクリック
3. **Create new deployment**をクリック
4. 以下を入力：
   - **Model**: `gpt-4`（またはご利用可能な最新モデル）
   - **Model version**: `最新`を自動選択
...
| `static/main.js` | API 呼び出し、Markdown 描画、Mermaid 抽出/変換ボタン | `fetch` + JSON で Flask ルートと通信、`marked` ライブラリで Markdown をレンダリング |
        ...
        <div class="main-content">
            ...
            <div class="panel">
                ...
                <textarea id="newsInput"
                    placeholder="ニュースの全文をここに貼り付けてください..."></textarea>
...
async function summarize() {
    ...
    if (!newsText) {
        alert('ニュースの本文を入力してください');
        return;
    }

    const summarizeBtn = document.getElementById('summarizeBtn');
    summarizeBtn.disabled = true;
    summarizeBtn.innerHTML = '<span class="loading"></span> 処理中...';
    ...
    try {
        ...
        if (response.ok) {
            ...
            data.mermaid_diagrams.forEach((diagram, index) => {
                ...
                btn.textContent = `図解 ${index + 1} を PNG で保存`;
...
async function convertAndDownloadDiagram(mermaidCode, diagramIndex) {
    ...
    try {
        ...
        btn.disabled = false;
        btn.innerHTML = `図解 ${diagramIndex + 1} を PNG で保存`;
    }
}
```
...
- **`fetch` + JSON でのルーティング**: すべてのリクエスト/レスポンス形式を JSON に揃え、`response.ok` を毎回チェックしています。エラーハンドリングを省略すると 500 エラーが利用者に伝わらず、「クリックしても何も起きない」という問い合わせにつながります。
- **Mermaid 図解のボタン生成**: `/extract-mermaid` で返った配列を動的にボタンへ変換し、必要なときだけ `convert-mermaid-png` を呼び出します。一度に全図解を変換しようとすると、1つのエラーで処理全体が止まってしまうため分割しています。
...
| **SUBSCRIPTION_KEY** | 同上 |
| **MODEL_NAME** | デプロイ時に指定した名前（例: `gpt-4`） |
...
1. [Azure Portal](https://portal.azure.com/)にサインイン
2. 検索バーで `news-summarizer-app-[ID]` を検索
...
| 項目 | 値 | 説明 |
|------|-----|------|
| **組織** | ご自分のGitHubユーザー名 | 例: `ken-kichi` |
| **リポジトリ** | `news-summarizer-p` | Step 4で作成したリポジトリ名 |
| **ブランチ** | `main` | デプロイするブランチ |

**設定例:**
- 組織: `アカウント名`
- リポジトリ: `news-summarizer-p`
- ブランチ: `main`
...
| MODEL_NAME | MODEL_NAME の値 | `gpt-4` |
...
#### エラー: `pip install failed`

**原因**: `requirements.txt` に問題がある
